...
Queries audit.db to produce a comprehensive performance report with a READY verdict.
...
def run_review(mode: str, days: int = 14) -> None:
    config  = load_config()
    ...
    total_decisions = sum(v for k, v in behaviour.items() if k in ("TRADE_BUY", "TRADE_SELL", "HOLD"))
    ...
    print(f"  Risk rejections: {behaviour.get('risk_rejections', 0)} ({behaviour.get('risk_rejection_rate_pct', 0)}% rejection rate)")
