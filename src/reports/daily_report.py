"""
daily_report.py — Daily P&L summary report.

Core logic extracted from scripts/daily_report.py so it can be called
...
Usage (standalone):
    python scripts/daily_report.py [--mode paper] [--days-ago N]
"""
...
def run_daily_report(mode: str, day_offset: int = 0, config: Optional[dict] = None) -> None:
    """Print a daily P&L report for *mode* (paper|live).
    ...
    AuditLogger(audit_db=audit_db, mode=mode)
