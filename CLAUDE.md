...
- **Volatility Windows & Dead Zones**: Only trades inside the `allowed_trading_hours` overlap (currently 03:00–04:00 SGT / 19:00–19:59 UTC, 1-hour window) and when current 15-min volume is strictly above `min_volume_ratio` of its 20-period moving average. `end_hour_utc` is exclusive. Volume dead zone check in `signals.py` is always active regardless of `enabled` flag. **`min_volume_ratio` is now per-pair**: BNB/UNI/INJ=0.30, TRX/DOGE/ADA/LTC/AVAX=0.40, BTC/ETH/SOL/XRP/SUI=0.50. Backtest forces `allowed_trading_hours.enabled=False` so all candles are evaluated.
...
- **ATR floor priority chain**: `indicators["adaptive_atr_floor_pct"]` (injected by main.py, rolling p25 ATR% × 0.8) → `pair_cfg["atr_tp_min_pct"]` → `dynamic_tp.atr_tp_min_pct` (0.30% fallback) → `trading.min_profit_floor_pct`. Adaptive injection enabled by `adaptive_atr_floor.enabled: true`; lookback=400 candles global, overrideable per-pair via `adaptive_atr_floor_lookback`.
...
- **Position sizing updated (Fix #116)**: `max_open_positions: 13`, `max_position_pct: 15%`, `max_buys_per_cycle: 5`. Agent can now deploy up to 90% of portfolio (6 positions × 15% each). `min_cash_reserve_pct: 10%` enforced.
