...
| session_2026_04_11t | Feat: drawdown recovery mode — restrict to BTC/ETH/BNB at half size when daily loss exceeds -3%; exit at -1.5% hysteresis; Telegram alerts on enter/exit; 7 tests (#182) |
...
- **Drawdown recovery mode (#182)**: When `daily_pnl_pct <= -trigger_pct` (default -3%), `validate_buy` Guard 1.2 blocks new buys on all non-major pairs (`BTC/USD`, `ETH/USD`, `BNB/USD`). Major pairs are allowed but capped at `max_position_pct_override` (default 10% of cash). Recovery exits when pnl recovers to > `-exit_pct` (default -1.5%). Hysteresis tracked in `loop_state["drawdown_recovery_active"]` — entry/exit Telegram alerts sent on state transitions only. Configured under `risk.drawdown_recovery` in `config.yaml`. Prevents the failure pattern: alts SL → circuit resets → agent re-enters alts → -7% kill switch panic-sell.
