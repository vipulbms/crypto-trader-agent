...
- **DB-persisted start-of-day balance (#170)**: `main.py` no longer captures `start_of_day_bal` once at startup. In paper mode, `_get_or_set_sod_balance(paper_db, current_balance)` is called at the start of each main loop iteration. It reads `agent_state` key `start_of_day_balance_YYYY-MM-DD` (UTC). If absent (new UTC day or `agent_state` was cleared by `reset_paper.py`), it writes `current_balance` and uses it. This prevents the false 40%+ daily loss notification that fires when `reset_paper.py` runs while the agent is alive. Also handles midnight rollovers automatically.
