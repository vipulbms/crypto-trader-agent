...
- **RAA universe decisions are LLM-delegated (#354)**: `propose_universe_addition()` is gone. `_run_llm_universe_decision()` runs a multi-turn tool loop (max 6 rounds, 6k token budget). The LLM calls `kraken_ticker`, `get_universe`, `get_trend_persistence`, `get_confidence_state` freely, then must call `universe_decision` with action=ADD/REMOVE/HOLD. All rules (persistence gate, alpha spread, persona gates) are in the system prompt. Two hard Python guards remain after the LLM decision: meme-block (S22.2.1) and HITL lock (S23.1.3). If the LLM fails to call `universe_decision`, the cycle defaults to HOLD and writes a `LLM_NO_DECISION` audit_feedback row. `RiskManager.validate_universe_proposal()` is unused by RAA but kept intact.
