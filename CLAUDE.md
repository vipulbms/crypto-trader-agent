...
- **ATR floor priority chain**: `indicators["adaptive_atr_floor_pct"]` (injected by main.py) → `pair_cfg["atr_tp_min_pct"]` → `dynamic_tp.atr_tp_min_pct` → `trading.min_profit_floor_pct`. The old global 1.0% has been reduced to 0.30% as fallback — per-pair values (0.12–0.37%) are the effective gates.
