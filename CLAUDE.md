...
| session_2026_05_24a | Fix: notifier initialization order in run_agent (#370); Feat: Persona-specific cycle intervals + Groq key rotation (S26) — CONSERVATIVE/MEDIUM/HIGH personas now have distinct `cycle_interval_minutes` (30/30/15 min), enabling dynamic frequency switching. Groq API keys rotated per cycle (even=primary, odd=secondary) to distribute 1M token budget across two independent 500k quotas. `get_active_persona_from_db()` added to cycle_context.py. Main loop reads active persona's cycle interval each cycle, not at startup. Closes #370 #371 |
...
- **Persona-specific cycle intervals + Groq key rotation (S26)**: Each persona now has a distinct `cycle_interval_minutes` in `config.yaml → personas:`. CONSERVATIVE: 30 min (0.15% daily), MEDIUM: 30 min (0.34% daily = 10x in 2 years), HIGH: 15 min (1% daily aggressive). `main.py` reads the active persona's cycle interval each cycle (not just at startup), enabling dynamic persona switching without agent restart. New helper `get_active_persona_from_db(db_path)` in `cycle_context.py` reads `active_persona_override` from `agent_state` table. **Groq key rotation**: When `GROQ_API_KEY_2` is set in environment, `TradingAgent._call_openai_compat()` rotates between primary and secondary keys based on `cycle_id % 2` (even=primary, odd=secondary). This distributes token usage across two independent 500k daily quotas (1M total). Keys are rotated at LLM call time, and the OpenAI client is recreated with the selected key. Rotation is logged: `[S26] Rotating to secondary Groq key (cycle N)`. Token budget at 15-min cycles (96/day): ~240k tokens/day ✅ well within 1M total.
